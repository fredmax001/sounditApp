...
const CreateEvent = () => {
  ...
  useEffect(() => {
    const checkSubscription = async () => {
      ...
      if (role !== 'business' && role !== 'organizer') return;
      ...
      try {
        ...
        if (!res.data?.has_subscription) {
          toast.warning(t('organizer.createEvent.subscriptionRequired') || 'Active subscription required');
  ...
  const uploadGallery = async (): Promise<string[]> => {
    ...
    try {
      const urls = await Promise.all(galleryFiles.map(file => uploadFile(file)));
      return urls;
    } finally {
  ...
  const handleSubmit = async (status: 'draft' | 'approved' = 'approved') => {
    ...
    if (!businessId) {
      toast.error(t('organizer.createEvent.businessProfileMissing'));
      return;
    }
    ...
    try {
      ...
      if (event && event.id) {
        for (const tier of ticketTiers) {
          await createTicketTierApi(Number(event.id), tier, authToken);
        }

        // Refresh event list so new event appears immediately
        try {
          await useEventStore.getState().fetchMyEvents();
        } catch {
          // non-blocking
        }
      ...
      toast.error(error instanceof Error ? error.message : t('organizer.createEvent.createError'));
    }
  };
    ...
    <div className="min-h-screen pb-24">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">
                  {formData.title || t('organizer.createEvent.newEvent')}
                </h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="px-5 py-2 bg-white/5 border border-white/10 text-white rounded-lg hover:bg-white/10 transition-colors disabled:opacity-50 font-medium"
              >
                Save Draft
              </button>
              <button
              ...
              >
                {isSubmitting ? (
                  <span className="flex items-center gap-2">
                    ...
                    {t('organizer.createEvent.publishing')}
                  </span>
                ) : (
                  t('organizer.createEvent.publish')
                )}
              </button>
            </div>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#0A0A0A]/95 backdrop-blur-md border-t border-white/10 md:hidden pb-safe">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <button
            onClick={() => handleSubmit('draft')}
            disabled={isSubmitting || !businessProfile}
            className="flex-1 py-2.5 bg-white/5 border border-white/10 text-white rounded-lg hover:bg-white/10 transition-colors disabled:opacity-50 font-medium text-sm"
          >
            Save Draft
          </button>
          <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <motion.div
          ...
          className="mb-8 p-4 bg-gradient-to-r from-[#d3da0c]/10 to-transparent border border-[#d3da0c]/20 rounded-xl flex items-center gap-3"
        >
          <div className="w-10 h-10 rounded-full bg-[#d3da0c]/20 flex items-center justify-center flex-shrink-0">
            <Upload className="w-5 h-5 text-[#d3da0c]" />
          </div>
          <div>
            <h3 className="text-white font-medium">{t('organizer.createEvent.createEventFaster')}</h3>
            <p className="text-gray-400 text-sm">{t('organizer.createEvent.createEventFasterDesc')}</p>
          </div>
        </motion.div>
        ...
        <div className="fixed inset-0 z-50 flex md:items-center md:justify-center bg-black/80 backdrop-blur-sm">
          ...
          >
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-white mb-1">{t('organizer.createEvent.eventCreated')}</h2>
            ...
            <div className="rounded-xl overflow-hidden border border-white/10 mb-6">
              {createdEvent.flyer_image ? (
                <img
                  src={createdEvent.flyer_image}
                  alt=""
              ...
              <div className="p-4">
                <h3 className="text-white font-semibold line-clamp-1">{createdEvent.title}</h3>
              </div>
            </div>
            ...
            {createdEvent.qr_code && (
              <div className="mb-6 text-center">
                ...
                <div className="bg-white rounded-xl p-4 inline-block">
                  <img src={createdEvent.qr_code} alt="QR" className="w-40 h-40" />
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              {createdEvent.qr_code && (
                <a
                  href={createdEvent.qr_code}
                  download={`event-${createdEvent.id}-qr.png`}
