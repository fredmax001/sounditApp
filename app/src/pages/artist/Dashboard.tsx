                ...
                <div className="col-span-2 lg:col-span-4">
                  <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-white sm:text-3xl lg:text-4xl">¥{stats.totalRevenue.toLocaleString()}</p>
                    </div>
                  </motion.div>
                </div>
                {[
                  ...
                  <div key={idx} className="lg:col-span-2 col-span-1">
                    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 * (idx + 1) }}
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold text-white sm:text-xl lg:text-2xl">{stat.value}</p>
                      </div>
                    </motion.div>
                  </div>
                ))}
                  ...
                  <section>
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      {incomingBookings.filter((b: Booking) => b.status === 'accepted').slice(0, 3).map((booking) => (
                        <div key={booking.id} className="bg-white/5 border border-white/5 p-3 rounded-xl flex items-center justify-between sm:p-4 lg:p-5 lg:rounded-2xl gap-3">
                          <div className="flex items-center gap-3 sm:gap-6 min-w-0">
                            <div className="w-10 h-10 sm:w-14 sm:h-14 bg-[#d3da0c]/10 rounded-xl flex items-center justify-center flex-shrink-0">
                              <Music className="w-5 h-5 sm:w-7 sm:h-7 text-[#d3da0c]" />
                            </div>
                            <div className="min-w-0">
                              <h4 className="text-white font-bold text-sm sm:text-lg truncate">{booking.event_name}</h4>
                          ...
                          <div className="text-right flex-shrink-0">
                            <p className="text-white font-bold text-sm sm:text-base lg:text-xl">¥{(booking.budget || 0).toLocaleString()}</p>
                            <span className="text-green-500 text-[10px] font-black uppercase tracking-widest hidden sm:inline">{t('artist.dashboard.status.confirmed')}</span>
                          </div>
                        </div>
                      ))}
                  ...
                  <section>
                    ...
                    <div className="space-y-4">
                      <motion.button
                        ...
                        className="w-full bg-white/5 p-4 sm:p-6 rounded-2xl border border-white/5 hover:border-[#d3da0c]/30 transition-all text-left"
                      >
                        <div className="flex items-center gap-3 sm:gap-4">
                          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#d3da0c]/20 rounded-xl flex items-center justify-center flex-shrink-0">
                            <Edit className="w-5 h-5 sm:w-6 sm:h-6 text-[#d3da0c]" />
                          </div>
                          <div className="min-w-0">
                            <h4 className="text-white font-bold text-sm sm:text-base">{t('artist.dashboard.updateProfile')}</h4>
                            <p className="text-gray-500 text-xs sm:text-sm">{t('artist.dashboard.updateProfileDesc')}</p>
                          </div>
                        </div>
                      </motion.button>
                      <motion.button
                        ...
                        className="w-full bg-white/5 p-4 sm:p-6 rounded-2xl border border-white/5 hover:border-purple-500/30 transition-all text-left"
                      >
                        <div className="flex items-center gap-3 sm:gap-4">
                          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-purple-500/20 rounded-xl flex items-center justify-center flex-shrink-0">
                            <Disc3 className="w-5 h-5 sm:w-6 sm:h-6 text-purple-500" />
                          </div>
                          <div className="min-w-0">
                            <h4 className="text-white font-bold text-sm sm:text-base">{t('artist.dashboard.manageMusicLinks')}</h4>
                            <p className="text-gray-500 text-xs sm:text-sm">{t('artist.dashboard.manageMusicLinksDesc')}</p>
                          </div>
                        </div>
                      </motion.button>
                    </div>
                ...
                <div className="grid gap-3 sm:gap-6">
                  {filteredBookings.map((booking) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="group flex flex-col items-start justify-between p-3 sm:p-4 bg-white/[0.02] border border-white/5 rounded-2xl lg:flex-row lg:items-center lg:p-6 lg:rounded-3xl hover:bg-white/[0.04] hover:border-[#d3da0c]/30 transition-all shadow-xl"
                    >
                      <div className="flex items-center gap-3 sm:gap-8 flex-1 min-w-0 w-full">
                        <div className="bg-[#d3da0c]/10 p-2.5 rounded-xl lg:p-4 lg:rounded-2xl group-hover:scale-110 transition-transform flex-shrink-0">
                          <Music className="w-6 h-6 sm:w-8 sm:h-8 text-[#d3da0c]" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 sm:gap-4 mb-1.5 sm:mb-2 flex-wrap">
                            <h4 className="text-white font-bold text-sm tracking-tight sm:text-base lg:text-xl truncate">{booking.event_name}</h4>
                            <span className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${booking.status === 'accepted' ? 'bg-green-500/10 text-green-500' :
                          ...
                          <div className="flex flex-wrap items-center gap-2 sm:gap-6 text-gray-500 text-xs sm:text-sm font-bold">
                            <span className="flex items-center gap-1.5 sm:gap-2">
                              <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#d3da0c]" />
                              {new Date(booking.proposed_date || booking.event_date).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })}
                            </span>
                            {booking.duration && (
                              <span className="flex items-center gap-1.5 sm:gap-2">
                                <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#d3da0c]" />
                                {booking.duration}h
                              </span>
                            )}
                            <span className="flex items-center gap-1.5 sm:gap-2">
                              <Users className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#d3da0c]" />
                              <span className="truncate">{booking.contact_name || t('artist.dashboard.eventOrganizer')}</span>
                            </span>
                          </div>
                          {booking.message && (
                            <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-black/40 rounded-xl border-l-4 border-[#d3da0c] max-w-2xl">
                              <p className="text-gray-400 text-xs sm:text-sm italic font-medium line-clamp-2">"{booking.message}"</p>
                            </div>
                          )}
                        </div>
                      </div>

                      <div className="flex items-center gap-4 sm:gap-10 mt-4 sm:mt-8 lg:mt-0 w-full lg:w-auto border-t lg:border-t-0 border-white/5 pt-4 sm:pt-8 lg:pt-0">
                        <div className="text-right">
                          ...
                          <p className="text-[#d3da0c] text-lg sm:text-xl font-bold tracking-tight lg:text-3xl">¥{(booking.budget || 0).toLocaleString()}</p>
                        </div>
                          ...
                          {booking.status === 'pending' ? (
                            <>
                              <button
                                ...
                                className="w-9 h-9 sm:w-11 sm:h-11 bg-red-500/10 text-red-500 rounded-xl touch-target lg:w-auto lg:h-auto lg:p-3 lg:rounded-2xl hover:bg-red-500 hover:text-white transition-all shadow-lg active:scale-95 flex items-center justify-center"
                              ...
                              <button
                                ...
                                className="px-3 py-2 sm:px-4 sm:py-2 bg-[#d3da0c] text-black font-bold rounded-xl text-xs sm:text-sm lg:px-6 lg:py-3 lg:rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-[0_0_20px_rgba(211, 218, 12,0.3)] flex items-center gap-2"
                              >
                                <Check className="w-5 h-5 sm:w-6 sm:h-6" />
                                <span className="hidden sm:inline">{t('artist.dashboard.acceptGig')}</span>
                                <span className="sm:hidden">Accept</span>
                              </button>
                            </>
                          ) : (
                            <button className="px-4 py-2 sm:px-8 sm:py-4 bg-white/5 border border-white/10 text-gray-400 font-black rounded-xl sm:rounded-2xl text-[10px] sm:text-xs uppercase tracking-widest disabled:opacity-50" disabled>
