...
const UserDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] pt-24 px-4 py-6 lg:p-10 lg:pt-28">
      <div className="flex flex-col items-start justify-between gap-4 mb-8 lg:flex-row lg:items-end lg:gap-8 lg:mb-12">
        <div className="flex items-center gap-4 lg:gap-8">
          <div className="relative group">
            ...
            {profile?.avatar_url ? (
              <img
                ...
                className="relative w-14 h-14 lg:w-24 lg:h-24 rounded-2xl lg:rounded-3xl object-cover border-2 border-white/10"
                onError={(e) => { (e.target as HTMLImageElement).src = '/default-avatar.png'; }}
              />
            ) : (
              <div className="relative w-14 h-14 lg:w-24 lg:h-24 rounded-2xl lg:rounded-3xl bg-gradient-to-br from-[#d3da0c] to-[#FF2D8F] flex items-center justify-center border-2 border-white/10">
                <span className="text-lg lg:text-3xl font-bold text-black">
                  {(profile?.first_name || t('user.dashboard.userAlt'))[0].toUpperCase()}
                </span>
              </div>
            )}
            <label className="absolute -bottom-1 -right-1 p-1.5 bg-[#d3da0c] rounded-lg lg:-bottom-2 lg:-right-2 lg:p-3 lg:rounded-2xl text-black shadow-xl hover:scale-110 active:scale-90 transition-all cursor-pointer">
              {isAvatarUploading ? (
          ...
          <div className="min-w-0">
            <h1 className="text-lg lg:text-3xl font-bold text-white tracking-tight mb-1 lg:mb-2">
              {t('user.dashboard.welcomeBackPrefix')}
              <span className="text-[#d3da0c]">{firstName}</span>
              {t('user.dashboard.welcomeBackSuffix')}
            </h1>
            <div className="flex items-center gap-3 text-gray-400 font-medium text-xs lg:text-sm">
        ...
        <div className="flex gap-2 lg:gap-3">
          <Link to="/user/settings">
            <button className="p-2.5 bg-white/5 rounded-xl lg:p-4 lg:rounded-2xl text-white hover:bg-white/10 transition-all relative touch-target">
              <Bell className="w-5 h-5 lg:w-6 lg:h-6" />
            </button>
          </Link>
          <Link to="/user/profile">
            <button className="flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white font-medium text-xs lg:px-8 lg:py-4 lg:rounded-2xl lg:font-bold lg:text-sm hover:bg-white/10 transition-all">
              <Edit className="w-4 h-4 lg:w-5 lg:h-5" />
              <span className="hidden lg:inline">{t('user.dashboard.editProfile')}</span>
      ...
      <div className="flex overflow-x-auto w-full gap-2 mb-6 bg-[#111111] p-1.5 rounded-2xl border border-white/5 shadow-2xl hide-scrollbar snap-tab-bar">
        {[
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-lg text-xs font-semibold lg:px-6 lg:py-3 lg:rounded-xl lg:text-sm lg:font-bold transition-all capitalize flex items-center gap-3 whitespace-nowrap ${activeTab === tab.id
        ...
        >
          {activeTab === 'explore' && (
            <div className="grid lg:grid-cols-3 gap-6 lg:gap-10">
              <div className="lg:col-span-2 space-y-10">
                <section className="relative overflow-hidden group rounded-2xl p-4 lg:rounded-3xl lg:p-8 bg-gradient-to-br from-[#111111] to-black border border-white/5">
                  {nextTicket ? (
                    <>
                      ...
                      <div className="relative z-20 max-w-md">
                        ...
                        <h2 className="text-xl lg:text-3xl font-bold text-white mb-3 tracking-tight">{nextTicket.event?.title || t('user.dashboard.yourNextEvent')}</h2>
                        ...
                        <button onClick={() => setActiveTab('tickets')} className="px-5 py-3 bg-[#d3da0c] text-black font-bold rounded-xl text-sm lg:px-8 lg:py-4 lg:rounded-2xl hover:scale-105 transition-all shadow-xl flex items-center gap-3">
                    ...
                    <>
                      ...
                      <div className="relative z-20 max-w-md">
                        ...
                        <h2 className="text-xl lg:text-3xl font-bold text-white mb-3 tracking-tight">{t('user.dashboard.findYourNext')} <span className="text-[#d3da0c]">{t('user.dashboard.experience')}</span></h2>
                        <p className="text-gray-400 text-sm font-medium mb-6">{t('user.dashboard.exploreEventsSubtitle')}</p>
                        <Link to="/events">
                          <button className="px-5 py-3 bg-[#d3da0c] text-black font-bold rounded-xl text-sm lg:px-8 lg:py-4 lg:rounded-2xl hover:scale-105 transition-all shadow-xl flex items-center gap-3">
                ...
                <section>
                  <div className="flex items-center justify-between mb-8">
                    <h3 className="text-base font-semibold text-white lg:text-xl">{t('user.dashboard.quickActions')}</h3>
                  </div>
                  <div className="grid grid-cols-2 gap-3 lg:grid-cols-2 lg:gap-4">
                    <Link to="/events">
                      <motion.div
                        whileHover={{ scale: 1.02 }}
                        className="bg-white/5 border border-white/5 rounded-2xl p-4 lg:rounded-3xl lg:p-6 group hover:border-[#d3da0c]/30 transition-all cursor-pointer"
                      >
                        <div className="flex items-center gap-6">
                          ...
                          <div>
                            <h4 className="text-white font-semibold text-sm lg:text-lg mb-1">{t('user.dashboard.browseEvents')}</h4>
                            <p className="text-gray-400 text-xs lg:text-sm font-semibold uppercase tracking-widest mt-1">{t('user.dashboard.findUpcomingParties')}</p>
                          </div>
                        </div>
                      </motion.div>
                    </Link>
                    <Link to="/artists">
                      <motion.div
                        whileHover={{ scale: 1.02 }}
                        className="bg-white/5 border border-white/5 rounded-2xl p-4 lg:rounded-3xl lg:p-6 group hover:border-purple-500/30 transition-all cursor-pointer"
                      >
                        <div className="flex items-center gap-6">
                          ...
                          <div>
                            <h4 className="text-white font-semibold text-sm lg:text-lg mb-1">{t('user.dashboard.discoverArtists')}</h4>
                            <p className="text-gray-400 text-xs lg:text-sm font-semibold uppercase tracking-widest mt-1">{t('user.dashboard.findYourFavoriteDjs')}</p>
                          </div>
                        </div>
                      </motion.div>
                    </Link>
              ...
              <div className="space-y-10">
                <section className="bg-[#111111] rounded-2xl border border-white/5 p-4 lg:rounded-3xl lg:p-6">
                  <h3 className="text-base font-semibold text-white lg:text-lg mb-8 border-b border-white/5 pb-4">{t('user.dashboard.saved')} <span className="text-[#d3da0c]">{t('user.dashboard.events')}</span></h3>
                ...
                <section className="bg-gradient-to-br from-purple-600/20 to-[#d3da0c]/20 rounded-2xl lg:rounded-[2.5rem] border border-white/5 p-6 lg:p-8 text-center relative overflow-hidden">
                  ...
                  <h3 className="relative z-10 text-base font-semibold text-white lg:text-xl mb-4">{t('user.dashboard.yourStats')}</h3>
                  <div className="grid grid-cols-2 gap-4 relative z-10">
                    <div className="bg-white/5 rounded-2xl p-4">
                      <p className="text-xl font-bold text-[#d3da0c] lg:text-3xl">{tickets.length}</p>
          ...
          {activeTab === 'tickets' && (
            <div className="space-y-4 lg:space-y-8">
              <div className="text-center">
                <h2 className="text-xl lg:text-3xl font-bold text-white mb-3 tracking-tight">{t('user.dashboard.yourEntry')} <span className="text-[#d3da0c]">{t('user.dashboard.passes')}</span></h2>
              ...
              {isTicketsLoading ? (
                ...
                <div className="text-center py-16 lg:py-20 bg-white/5 rounded-2xl lg:rounded-[3rem] border border-white/10">
                  ...
                  <h3 className="text-base font-semibold text-white lg:text-lg mb-2">{t('user.dashboard.noTicketsYet')}</h3>
                ...
                <div className="grid gap-10">
                  {upcomingTickets.map(ticket => (
                    <div key={ticket.id} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden flex-col lg:rounded-3xl shadow-2xl">
                      ...
                      <div className="flex-1 p-6 lg:p-10 flex flex-col items-center gap-6 lg:gap-10">
                        <div className="flex-1 space-y-4 lg:space-y-6 text-center">
                          <div className="grid grid-cols-2 gap-3 lg:gap-6">
                            <div>
                              ...
                              <p className="text-white font-semibold text-sm lg:text-lg mb-1">{ticket.date}</p>
                            </div>
                            <div>
                              <p className="text-gray-500 text-[10px] font-bold uppercase tracking-[0.2em] mb-2">{t('user.dashboard.ticketNumber')}</p>
                              <p className="text-white font-semibold text-sm lg:text-lg mb-1">{ticket.qr}</p>
                            </div>
                          </div>
                        ...
                        <div className="w-28 h-28 lg:w-40 lg:h-40 bg-white p-3 lg:p-4 rounded-2xl lg:rounded-3xl flex items-center justify-center shadow-[0_0_40px_rgba(255,255,255,0.1)]">
          ...
          {activeTab === 'bookings' && (
            <div className="space-y-6 lg:space-y-10">
              <div className="text-center">
                <h2 className="text-xl lg:text-3xl font-bold text-white mb-3 tracking-tight">{t('user.dashboard.myBookings')}</h2>
              ...
              {bookingsLoading ? (
                ...
                <div className="text-center py-16 lg:py-20 bg-white/5 rounded-2xl lg:rounded-[3rem] border border-white/10">
                  ...
                  <h3 className="text-base font-semibold text-white lg:text-lg mb-2">{t('user.dashboard.noBookingsYet')}</h3>
                ...
                <div className="grid gap-6">
                  {bookings.map(booking => {
                    ...
                    return (
                      <div key={booking.id} className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-3 lg:p-6 lg:gap-4 items-start">
                        ...
                        <div className="flex-1">
                          <h3 className="text-base font-semibold text-white lg:text-lg mb-1">{booking.event_name || t('user.dashboard.untitledEvent')}</h3>
          ...
          {activeTab === 'saved' && (
            <div className="space-y-10">
              <div className="text-center mb-10">
                <h2 className="text-xl lg:text-3xl font-bold text-white mb-3 tracking-tight">{t('user.dashboard.yourSaved')} <span className="text-[#d3da0c]">{t('user.dashboard.events')}</span></h2>
              ...
              {loadingSaved ? (
                ...
                <div className="text-center py-20">
                  ...
                  <h3 className="text-base font-semibold text-white lg:text-lg mb-2">{t('user.dashboard.noSavedEvents')}</h3>
                ...
                <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-6">
                  {savedEvents.map((item: SavedEvent, idx: number) => (
                    ...
                    >
                      <div className="relative h-28 lg:h-40 overflow-hidden">
                      ...
                      <div className="p-3 space-y-4 lg:p-4">
                        <div>
      ...
      {uploadModalBooking && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#141414] rounded-2xl p-4 max-w-md w-full mx-4 border border-white/10 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleUploadPayment} className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-[#d3da0c] text-black py-3 rounded-xl font-semibold text-sm lg:py-4 hover:bg-[#bbc10b] transition-colors disabled:opacity-50"
              >
